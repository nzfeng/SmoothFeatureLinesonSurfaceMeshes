...
bool callback_key_down(Viewer &viewer, unsigned char key, int modifiers) {
  ...
  if (key == '2') {
    ...
    viewer.data().clear();
    viewer.data().set_mesh(V, F);
    viewer.data().add_edges(V, V + vScale * EV[0],
                            Eigen::RowVector3d(0.9, 0.1, 0.1));
    viewer.data().add_edges(V, V + vScale * EV[1],
                            Eigen::RowVector3d(0.1, 0.9, 0.1));
  ...
  if (key == '5') {
      ...
      for (int f = 0; f < F.rows(); ++f)
        if (is_regular[k](f)) {
          ...
          for (int j = 0; j < F.cols(); ++j) {
            ...
            c.push_back(GM::Cd(V.row(i).dot(my_basis[0].row(f)),
                               V.row(i).dot(my_basis[1].row(f))));
          }
          /// make consistent
          ...
          auto e_gradient = e_gradient_cd.real() * my_basis[0].row(f) +
                            e_gradient_cd.imag() * my_basis[1].row(f);
          /// check equation (6)
          ...
          for (int fi1 = 0; fi1 < F.cols(); ++fi1) {
            ...
            if (GM::sgn(e_slice(fi1)) * GM::sgn(e_slice(fi2)) < 0) {
              ...
              if (zero_id.count(make_pair(F(f, fi1), F(f, fi2))))
                ...
                /// new point
                point_id = n_points;
                zero_id[make_pair(F(f, fi1), F(f, fi2))] =
                    zero_id[make_pair(F(f, fi2), F(f, fi1))] = point_id;
                ++n_points;
                z.push_back((V.row(F(f, fi1)) * fabs(e_slice(fi2)) +
                             V.row(F(f, fi2)) * fabs(e_slice(fi1))) /
                            (fabs(e_slice(fi2)) + fabs(e_slice(fi1))));
                ZW[k].push_back((K[k](F(f, fi1)) * fabs(e_slice(fi2)) +
                                 K[k](F(f, fi2)) * fabs(e_slice(fi1))) /
                                (fabs(e_slice(fi2)) + fabs(e_slice(fi1))));
    ...
    for (int k = 0; k < (int)K.size(); ++k) {
      ...
      Eigen::MatrixXd endpoints1(total_degree / 2, ZV[k].cols()),
          endpoints2(total_degree / 2, ZV[k].cols());
  ...
  if (key == '6') {
    ...
    for (int k = 0; k < (int)K.size(); ++k) {
        ...
        if (zero_adj[k][i].size() != 2) {
          ...
          for (int t : zero_adj[k][i]) {
            ...
            for (int j = 0; j + 1 < (int)points_inside.size(); ++j) {
              total_weight +=
                  0.5 *
                  (ZW[k][points_inside[j]] + ZW[k][points_inside[j + 1]]) *
                  (ZV[k].row(points_inside[j]) -
                   ZV[k].row(points_inside[j + 1]))
                      .norm();
            }
            if ((k == 0 && total_weight > threshold * vScale * kScale) ||
                (k == 1 && total_weight < threshold * vScale * kScale)) {
              for (int j = 0; j + 1 < (int)points_inside.size(); ++j)
      ...
      auto color = Eigen::RowVector3d(0.1, 0.1, 0.1);
      color(k) = 0.9;
      viewer.data().add_edges(GM::toEigenMatrix(endpoints1),
                              GM::toEigenMatrix(endpoints2), color);
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  for (int k = 0; k < (int)K.size(); ++k) {
    ...
    auto gradient_complex = GM::get_gradient(
        V, F, 3.0 * K[k].array() / area_star.array(), my_basis);
    auto gradient = GM::to_vector_field(gradient_complex, my_basis);
    GM::compute_extremalities(V, F, gradient, EV[k], is_regular[k],
                              extremalities[k]);
